...
async fn setup_storage(
    ...
    storage_writer::write_storage_logs(ctx, pool.clone(), insert_protective_reads).await;
    genesis_params
}
...
async fn run_playground(
    ...
    let genesis_params = setup_storage(ctx, &pool, 5, insert_protective_reads).await;
    ...
    wait_for_all_batches(ctx, playground, playground_tasks, &mut conn).await;
}
...
async fn wait_for_all_batches(
    ...
    scope::run!(ctx, |ctx,s| async {
        s.spawn_bg(playground_tasks.output_handler_factory_task.run(ctx));
        s.spawn_bg(playground.run(ctx));
        if let Some(loader_task) = playground_tasks.loader_task {
            s.spawn_bg(loader_task.run(ctx));
        }
...
async fn starting_from_non_zero_batch(storage_loader_kind: StorageLoaderKind) {
    ...
    run_playground(ctx, pool, storage_loader, Some(L1BatchNumber(3))).await;
}
...
async fn using_larger_window_size(window_size: u32) {
    ...
    let genesis_params = setup_storage(ctx, &pool, 5, false).await;
    let cursor = VmPlaygroundCursorOptions {
