...
use zksync_types::{
    ethabi, fee::Fee, l2::L2Tx, transaction_request::PaymasterParams,
    Address, Nonce, U256,
};
...
pub struct MintERC20Builder<'a, S: EthereumSigner, P> {
    wallet: &'a Wallet<S, P>,
    to: Option<Address>,
...
impl<'a, S, P> MintERC20Builder<'a, S, P>
where
    S: EthereumSigner,
    P: ZksNamespaceClient + EthNamespaceClient + Sync,
{
    ...
    //         ];
    //         let mut calldata = ethabi::short_signature("mint", &calldata_params).to_vec();
    ...
    async fn get_execute_builder(&self) -> Result<ExecuteContractBuilder<'_, S, P>, ClientError> {
        // Ensure token is provided and not zero
        let token = self
            .token
            .ok_or_else(|| ClientError::MissingRequiredField("token".into()))?;
        if token.is_zero() {
        ...
        // Construct calldata for the `mint` function
        let calldata_params = vec![
            ethabi::ParamType::Address, // Recipient address
            ethabi::ParamType::Uint(256), // Amount to mint
        ];
        let mut calldata = ethabi::short_signature("mint", &calldata_params).to_vec();
        calldata.append(&mut ethabi::encode(&[
            ethabi::Token::Address(to),
        ...
        // Use ExecuteContractBuilder with the constructed calldata
        let paymaster_params = self.paymaster_params.clone().unwrap_or_default();
        let mut builder = ExecuteContractBuilder::new(self.wallet)
            .contract_address(token) // Use token address as the contract
            .calldata(calldata)
            .paymaster_params(paymaster_params);
    
        // Optional: Add fee and nonce if available
        if let Some(fee) = self.fee.clone() {
            builder = builder.fee(fee);
        }
        if let Some(nonce) = self.nonce {
    ...
    /// Same as `MintERC20Builder::to`, but accepts a string address value.
    ///
