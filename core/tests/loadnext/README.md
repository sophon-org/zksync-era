...
For direct L2 txs using a paymaster with native token (non-erc20):

- `SKIP_L1_OPERATIONS` should be true
- No need for `MAIN_TOKEN` or `L1_` parameters
- L2_MAIN_TOKEN can be omitted for now -would be user for erc20 transfers-
- `LOADNEXT_CONTRACT` read array size should match `CONTRACT_EXECUTION_PARAMS_READS`

```shell
(set -a && source .env && set +a && \
cargo build && \
L2_RPC_ADDRESS=$L2_RPC_ADDRESS \
L2_WS_RPC_ADDRESS=$L2_WS_RPC_ADDRESS \
L2_CHAIN_ID=$L2_CHAIN_ID \
L2_MAIN_TOKEN=$L2_MAIN_TOKEN \
MAIN_TOKEN=$MAIN_TOKEN \
PAYMASTER_ADDRESS=$PAYMASTER_ADDRESS \
LOADNEXT_CONTRACT=$LOADNEXT_CONTRACT \
SKIP_L1_OPERATIONS=$SKIP_L1_OPERATIONS \
